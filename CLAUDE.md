...
| 语言 | Swift 6.0（严格并发） |
| UI 框架 | SwiftUI |
| Markdown 渲染 | [Textual](https://github.com/gonzalezreal/textual) v0.3.1+（过渡期保留，v2.x 将迁移至 cmark-gfm + WKWebView） |
...
│   ├── ThemeDefinition.swift      # 5 色主题 + 23 预设 + 自定义覆盖
...
│   ├── RenderedMarkdownView.swift  # Textual StructuredText 渲染
...
唯一外部依赖：**Textual** v0.3.1+（MarkdownUI 官方继任者）
